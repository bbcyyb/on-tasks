...
function pollerMessageCacheJobFactory(
    ...
    PollerMessageCacheJob.prototype._run = function run() {
	   ...
	   _.forEach([
            ...
            function(command) {
                self._subscribeWsmanCommandResult(
                self.routingKey, 
                command, 
                self.createSetWsmanCommandResultCallback(command)
              );
         });
     ...
     * @memberOf PollerMessageCacheJob
     */
    PollerMessageCacheJob.prototype.createSetWsmanCommandResultCallback = function(command) {
        var self = this;
        return function(data) {
        	self.cacheSet(data.workItemId || 'unknown.wsman.' + command,
                _.omit(data, 'workItemId'
            ));
        };
    }; 
