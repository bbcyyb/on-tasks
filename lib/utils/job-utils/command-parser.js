...
function commandParserFactory(Logger, Promise, _) {
    ...
    CommandParser.prototype[ls]= function(data) {
        ...
        try {
            // return Promise.resolve({ source: 'ls', data: data, store: true});

            let result = _parseLsCommandToJson(data);
            return Promise.resolve({ source: 'ls', data: result, store: true });
        } catch(err) {
            return Promise.resolve({ source: 'ls', error: err});
        }
    };
    ...
    function _parseLsCommandToObject(data) {
        const period = 9;
        const prefix_ = 2;
        ...
        for(var i = 0; i < array.length; i++) {
            ...
            var col = (i - prefix_) % period;
            var row = parseInt((i - prefix_) / period);
            var prevRow = parseInt((i - prefix_ - 1) < 0 ? -period : (i - prefix_ - 1) / period);
            if(row != prevRow){
                entity.array.push({});
            }

            var rowEntity = entity.array[entity.array.length - 1];

            switch(col){
                case 0:
                    let right = array[i];
                    let type = right.substr(0, 1);
                    let owner = right.substr(1, 3);
                    let group = right.substr(4, 3);
                    let other = right.substr(7, 3);
                    rowEntity.right = {type: type, owner: owner, group: group, other: other};
                    break;
                case 1:
                    rowEntity.filecount = array[i];
                    break;
                case 2:
                    rowEntity.user = array[i];
                    break;
                case 3:
                    rowEntity.group = array[i];
                    break;
                case 4:
                    rowEntity.size = array[i];
                    break;
                case 5:
                    break;
                case 6:
                    break;
                case 7:
                    rowEntity.datetime = array[i-2] + " " + array[i-1] + " " + array[i];
                    break;
                case 8:
                    rowEntity.name = array[i];
                    break;
                default:
                    break;
            }
    ...
    function  _parseLsCommandToJson(data){
         let entity = _parseLsCommandToObject(data);
